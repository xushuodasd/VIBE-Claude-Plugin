...
由于 Claude Code 的安全机制与插件规范，本项目现在按**标准插件**方式分发。`/vibe` 只有在插件被正确安装到 Claude Code 插件目录后才会出现。

### 官方推荐安装方式 (Marketplace)

**第一步：配置 Marketplace 源**
打开你的 Claude Code 全局配置文件 `~/.claude/settings.json`（Windows 下为 `%USERPROFILE%\.claude\settings.json`），在 `extraKnownMarketplaces` 中添加 `vibe` 源：
...
**第二步：在 Claude Code 中安装**
重启 Claude Code，在聊天框中输入以下指令一键安装：
```bash
...
如果你不想修改配置文件，可以直接将本仓库克隆到 Claude Code 的全局插件目录下。注意：必须克隆**整个仓库**，不能只复制 `skills/` 目录。
...
克隆完成后，**重启你的 Claude Code** 即可生效。重启后建议先执行：

```bash
/plugins
```

确认列表里能看到 `vibe-claude-plugin`，然后再执行 `/vibe`。

## 🚀 开启全自动开发 (Usage)

为了实现真正的零点击挂机，请在启动时信任终端（`claude --trust`）。

在聊天框中，你现在可以直接使用**专属的斜杠命令**来一键唤醒引擎：

```bash
/vibe 我要从头做一个前端待办事项网站。要求深色工业风，按钮有物理反馈感。后端用 Node.js 存 SQLite。
```

*(如果你没有加具体需求，只输入 `/vibe`，AI 会主动开始和你沟通需求。)*

如果重启后 `/vibe` 仍然不存在，请先执行 `/plugins` 检查插件是否真的已加载；如果列表里没有 `vibe-claude-plugin`，说明安装路径或插件缓存有问题，而不是命令本身的问题。
